...
const FROM = process.env.GOOGLE_CALENDAR_IMPERSONATE_EMAIL ?? 'reservations@conferra.co';

interface BookingConfirmationInput {
    to: string[];               // organizer + attendees
    bookingId: string;
    title: string;
    roomName: string;
    floor: number;
...
export async function sendBookingConfirmationEmail(input: BookingConfirmationInput): Promise<void> {
    ...
    const meetLinkBlock = input.meetLink
        ...
        `
        : '<p style="color: #888; font-size: 13px;">No video link attached.</p>';
    ...
    const html = `
        <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; padding: 32px; color: #111;">
            <div style="margin-bottom: 28px;">
                ...
                <p style="color: #555; font-size: 14px; margin: 0;">Ref: <code style="font-family: monospace;">${input.bookingId}</code></p>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 28px;">
                ...
                <tr>
                    ...
                    <td style="padding: 10px 0; border-bottom: 1px solid #eee; font-size: 14px;">${input.startTime} – ${input.endTime}</td>
                </tr>
                <tr>
                    <td style="padding: 10px 0; color: #888; font-size: 13px;">Organizer</td>
            ...
            <p style="color: #aaa; font-size: 12px; margin: 0;">
                This confirmation was sent by Conferra. If you have questions, reply to this email.
            </p>
        </div>
    `;
    ...
    const { error } = await resend.emails.send({
        from: `Conferra <${FROM}>`,
        to: allRecipients,
        subject: `Confirmed: ${input.title} on ${formatDate(input.date)}`,
        html,
    });
